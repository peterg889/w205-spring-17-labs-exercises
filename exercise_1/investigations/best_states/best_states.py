...
conf = SparkConf().setAppName("best states")
...
combined = joined.map(lambda x : (x[0], x[1][0][1],x[1][0][2], x[1][1][1]))

# get total score per state
states = combined.map(lambda x : ((x[2],) , x[3]))
summed_states = states.reduceByKey(lambda x,y:x+y)

# count the number of states
state_count = combined.map(lambda x : ((x[2],), 1))
summed_state_count = state_count.reduceByKey(lambda x,y:x+y)

sum_and_count = summed_states.join(summed_state_count)

# calc average
average_score = sum_and_count.map(lambda x : (x[0][0], x[1][0]/x[1][1]))

sorted_states = average_score.sortBy(lambda x : -x[1])
top = sorted_states.take(10)
